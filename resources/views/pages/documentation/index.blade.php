    ...
    <div class="space-y-6" x-data="{ tab: 'training' }">
        <header class="page-hero">
            ...
            <div class="mt-4 flex flex-wrap gap-2">
                <a href="{{ route('documentation.materials') }}" wire:navigate class="rounded-lg bg-indigo-600 px-3 py-2 text-sm font-medium text-white hover:bg-indigo-700">Kelola Materi</a>
                <a href="{{ route('documentation.support-contacts') }}" wire:navigate class="rounded-lg border border-white/20 bg-white/10 px-3 py-2 text-sm font-medium text-white backdrop-blur-sm hover:bg-white/20">Kelola Support Contacts</a>
                <a href="{{ route('documentation.faqs') }}" wire:navigate class="rounded-lg border border-white/20 bg-white/10 px-3 py-2 text-sm font-medium text-white backdrop-blur-sm hover:bg-white/20">Kelola FAQ</a>
            </div>
        </header>

        <div class="inline-flex rounded-full bg-white p-1 shadow-sm ring-1 ring-zinc-200 dark:bg-zinc-900 dark:ring-zinc-700">
            <button type="button" @click="tab = 'training'" :class="tab === 'training' ? 'bg-indigo-600 text-white shadow' : 'text-zinc-600 hover:text-zinc-900 dark:text-zinc-300 dark:hover:text-white'" class="rounded-full px-5 py-2 text-sm font-medium transition">
                Training
            </button>
            <button type="button" @click="tab = 'support'" :class="tab === 'support' ? 'bg-indigo-600 text-white shadow' : 'text-zinc-600 hover:text-zinc-900 dark:text-zinc-300 dark:hover:text-white'" class="rounded-full px-5 py-2 text-sm font-medium transition">
                Support
            </button>
        </div>
            ...
            <section class="rounded-2xl border border-zinc-200 bg-white p-5 shadow-sm dark:border-zinc-700 dark:bg-zinc-900">
                ...
                <div class="mt-3 grid gap-3 md:grid-cols-2 lg:grid-cols-3">
                    @forelse ($materials as $material)
                        <article class="rounded-xl border border-zinc-200 p-4 transition hover:-translate-y-0.5 hover:border-indigo-200 hover:shadow-md dark:border-zinc-700">
                            <div class="font-mono text-[11px] font-semibold uppercase tracking-[0.14em] text-zinc-400">{{ $material->material_type }}</div>
                            ...
                            @if ($material->content_url)
                                <a href="{{ $material->content_url }}" target="_blank" class="mt-3 inline-block text-sm text-indigo-600 hover:underline">Buka Materi</a>
                            @elseif ($material->file_path)
                                <a href="{{ Storage::url($material->file_path) }}" target="_blank" class="mt-3 inline-block text-sm text-indigo-600 hover:underline">Buka File PDF</a>
                            @endif
                        </article>
                    @empty
            ...
            <section class="rounded-2xl border border-zinc-200 bg-white p-5 shadow-sm dark:border-zinc-700 dark:bg-zinc-900">
                ...
                <div class="mt-3 grid gap-3 md:grid-cols-3">
                    @forelse ($supportContacts as $contact)
                        <div class="rounded-xl border border-zinc-200 p-4 transition hover:-translate-y-0.5 hover:border-indigo-200 hover:shadow-md dark:border-zinc-700">
                            <div class="font-mono text-[11px] font-semibold uppercase tracking-[0.14em] text-zinc-400">{{ str_replace('_', ' ', $contact->type) }}</div>
                            <div class="mt-1 font-medium">{{ $contact->label }}</div>
                            @if ($contact->contact_url)
                                <a href="{{ $contact->contact_url }}" target="_blank" class="mt-1 block text-sm text-indigo-600 hover:underline">{{ $contact->contact_value }}</a>
                            @else
